...
# list = [f(i) for i in range(1, 21) if i % 2 == 0]   # Также мы можем обрабатывать даные тоесть указывать не просто само
#                                                     #    значения, а могли взять кое какую функцию (делаем по старинке) есть
#                                                     #    функция f которая будет возводит число в 3 степень в вырожения строке
#                                                     #    40 подписываем вырожения f(i) что сейчас произойдет у нас будет
#                                                     #    перебераться все числа в диапозоне от 1 до 20 из них будут выброны
#                                                     #    только те значения для которых числа четные после этого будет к
#                                                     #    определеной выборке применяться функция f и будит возводит втретию
#                                                     #    степень
